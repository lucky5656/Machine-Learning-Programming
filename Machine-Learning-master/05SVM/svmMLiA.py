...
def testRbf(k1=1.3):
    ...
    b, alphas = smoP(dataArr, labelArr, 0.6, 0.001, 40, ('rbf', k1))  # 根据训练集计算b和alphas
    datMat = np.mat(dataArr)
    labelMat = np.mat(labelArr).transpose()
    svInd = np.nonzero(alphas.A > 0)[0]  # 获得支持向量 ,np.nonzero函数是numpy中用于得到数组array中非零元素的位置（数组索引）的函数
    sVs = datMat[svInd]
