...
def scrapePage(retX, retY, inFile, yr, numPce, origPrc):
    ...
    with open(inFile, encoding='utf-8') as f:
        html = f.read()
    soup = BeautifulSoup(html, "lxml")  # 创建Beautiful Soup对象,指定lxml解析器解析

    i = 1
    # 根据HTML页面结构进行解析
    currentRow = soup.find_all('table', r="%d" % i)  # find_dall总返回一个列表,如果没有找到则返回空
...
if __name__ == '__main__':
    lgX = []
    lgY = []
    setDataCollect(lgX, lgY)  # 获取数据

    useStandRegres()  # 使用简单的线性回归

    crossValidation(lgX, lgY)  # 使用岭回归

    print(ridgeTest(lgX, lgY))  # 看一下在缩减过程中回归系数是如何变化的
